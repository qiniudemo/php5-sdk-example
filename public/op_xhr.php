...
if($code != 200) {
    echo json_encode(array("code" => $code, "data" => array("errmsg" => QBox_ErrorMessage($code, $error))));
    exit(-1);
}
$opURL = $result['url'];
...
switch ($act) {
    case "image_info":
        $url = QBox_FileOp_ImageInfoURL($opURL);
        break;
    case "image_preview":
        $thumbType = isset($_POST["type"]) ? (int)trim($_POST["type"]) : 1;
        $url = QBox_FileOp_ImagePreviewURL($opURL, $thumbType);
        break;
    case "make_style":
        ...
        $url = QBox_FileOp_StylePreviewURL($opURL, $templPngFile, $paramStr, $quality);
        break;
    default:
